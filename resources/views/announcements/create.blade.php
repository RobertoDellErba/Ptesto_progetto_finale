  ...
  <div class="container  bg_second rounded shadow my-2">
    ...
    <div class="row py-3">
      ...
      <div class="col-12">
        ...
        <form action="{{route('announcements.store')}}" method="POST" class="card shadow border-0 rounded p-2 p-md-5 bg_light">
          @csrf
        <input type='hidden' name="uniqueSecret" value="{{$uniqueSecret}}">
          <div class="form-row mb-3">
            <div class="col-md-6 mb-3">
              <label class="text-footer font-weight-bold lead" for="nomeProdotto " >{{__('ui.product_name')}}</label>
              <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="nomeProdotto"   placeholder="Inserire qui nome prodotto..." value="{{old('title')}}">
              @error('title')
          ...
          <div class="form-row mb-3">
            <div class="col-12 mb-3">
              ...
              <textarea type="text" name="body" id="mytextarea" class="form-control @error('body') is-invalid @enderror" cols="30" rows="3">{{old('body')}}</textarea>
              @error('body')
              <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="form-row mb-3">
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="number" step="0.01" name="price" id="prezzoProdotto" min="0" class="form-control @error('price') is-invalid @enderror" placeholder="€€€" value="{{old('price')}}">
              @error('price')
              <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>                      
          ...
          <div class="form-row mb-3">
            <div class="col-12">
              ...
              <div class="col-12">
                <div class="dropzone @error('images') is-invalid @enderror" id="drophere"></div>
                @error('images')
                <div class="alert alert-danger">{{$message}}</div>
                @enderror
              </div>
          ...
          {{-- <div class="form-group">
            <div class="form-check">
              <input class="form-check-input text-footer" type="checkbox" value="" id="invalidCheck2" required>
              <label class="form-check-label " for="invalidCheck2">
                Accetto i termini di sicurezza
              </label>
            </div>
          </div> --}}
